...
public class CbtLogs {
    ...
    private static void printDeviceInfo(final String filePath) {
        ...
        int versionCode = 0;
        try {
            PackageInfo pi = CbtManager.getInstance().getContext()
                    .getPackageManager()
                    .getPackageInfo(CbtManager.getInstance().getContext().getPackageName(), 0);
            if (pi != null) {
    ...
    public static class Config {
        ...
        private Config() {
            ...
            if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())
                    && CbtManager.getInstance().getContext().getExternalCacheDir() != null)
                mDefaultDir = CbtManager.getInstance().getContext().getExternalCacheDir() + FILE_SEP + "log" + FILE_SEP;
            else {
                mDefaultDir = CbtManager.getInstance().getContext().getCacheDir() + FILE_SEP + "log" + FILE_SEP;
            }
        }
